...
pub const RegexTargetExclusion = struct {
    selector: RuleExclusion,
    collection: []const u8,
    compiled: *selectors.RegexSelector,
    worker: selectors.RegexSelector.Worker,
};

fn freeRuleExclusion(allocator: std.mem.Allocator, exclusion: RuleExclusion) void {
    switch (exclusion) {
        .id => {},
...
fn deinitRuleExclusions(allocator: std.mem.Allocator, exclusions: *std.ArrayList(RuleExclusion)) void {
    for (exclusions.items) |exclusion| freeRuleExclusion(allocator, exclusion);
...
fn deinitTargetExclusions(allocator: std.mem.Allocator, exclusions: *std.ArrayList(TargetExclusion)) void {
    for (exclusions.items) |exclusion| {
        freeRuleExclusion(allocator, exclusion.selector);
        allocator.free(exclusion.target);
    }
    exclusions.deinit(allocator);
}

fn deinitRegexTargetExclusions(allocator: std.mem.Allocator, exclusions: *std.ArrayList(RegexTargetExclusion)) void {
    for (exclusions.items) |*exclusion| {
        exclusion.worker.deinit();
        exclusion.compiled.deinit();
        allocator.destroy(exclusion.compiled);
        freeRuleExclusion(allocator, exclusion.selector);
        allocator.free(exclusion.collection);
    }
    exclusions.deinit(allocator);
}
...
pub const Transaction = struct {
    ...
    regex_target_exclusions: std.ArrayList(RegexTargetExclusion) = .empty,
    ...
    fn applyMatchedRules(
        ...
        var staged_regex_target_exclusions: std.ArrayList(RegexTargetExclusion) = .empty;
        defer deinitRegexTargetExclusions(self.waf.allocator, &staged_regex_target_exclusions);
        ...
        result.controls_applied = try self.stageRuntimeControls(
            ...
            &staged_regex_target_exclusions,
        ...
        try self.regex_target_exclusions.ensureUnusedCapacity(self.waf.allocator, staged_regex_target_exclusions.items.len);
        ...
        self.regex_target_exclusions.appendSliceAssumeCapacity(staged_regex_target_exclusions.items);
        ...
        staged_regex_target_exclusions.clearRetainingCapacity();
    ...
    fn stageRuntimeControls(
        ...
        staged_regex_target_exclusions: *std.ArrayList(RegexTargetExclusion),
            ...
            for (plan.runtime_controls[rule.controls_start..][0..rule.controls_count]) |control| {
                ...
                switch (control.kind) {
                    ...
                    .rule_remove_target_by_id, .rule_remove_target_by_tag => {
                        ...
                        const runtime_target = action_config.parseRuntimeTarget(parsed.target) catch return error.InvalidActionValue;
                        const selector_bytes = if (control.kind == .rule_remove_target_by_tag) parsed.selector.len else 0;
                        const target_bytes = switch (runtime_target) {
                            .exact => |value| value.len,
                            .regex => |value| std.math.add(usize, value.collection.len, value.pattern.len) catch return error.CapacityExceeded,
                        };
                        const added_bytes = std.math.add(usize, selector_bytes, target_bytes) catch return error.CapacityExceeded;
                        if (self.rule_exclusions.items.len + self.target_exclusions.items.len + self.regex_target_exclusions.items.len +
                            staged_exclusions.items.len + staged_target_exclusions.items.len + staged_regex_target_exclusions.items.len ==
                            self.waf.config.limits.max_runtime_exclusions or
                        ...
                        const selector: RuleExclusion = if (control.kind == .rule_remove_target_by_id)
                            ...
                            const owned_tag = try self.waf.allocator.dupe(u8, parsed.selector);
                            break :blk .{ .tag = owned_tag };
                        };
                        switch (runtime_target) {
                            .exact => |target| {
                                const owned_target = self.waf.allocator.dupe(u8, target) catch |err| {
                                    freeRuleExclusion(self.waf.allocator, selector);
                                    return err;
                                };
                            ...
                            .regex => |target| {
                                const owned_collection = self.waf.allocator.dupe(u8, target.collection) catch |err| {
                                    freeRuleExclusion(self.waf.allocator, selector);
                                    return err;
                                };
                                const compiled = self.waf.allocator.create(selectors.RegexSelector) catch |err| {
                                    freeRuleExclusion(self.waf.allocator, selector);
                                    self.waf.allocator.free(owned_collection);
                                    return err;
                                };
                                compiled.* = selectors.RegexSelector.compile(self.waf.allocator, target.pattern) catch |err| {
                                    self.waf.allocator.destroy(compiled);
                                    freeRuleExclusion(self.waf.allocator, selector);
                                    self.waf.allocator.free(owned_collection);
                                    return switch (err) {
                                        error.OutOfMemory => error.OutOfMemory,
                                        else => error.InvalidActionValue,
                                    };
                                };
                                var worker = compiled.worker();
                                staged_regex_target_exclusions.append(self.waf.allocator, .{
                                    .selector = selector,
    ...
    pub fn targetExcluded(self: *Transaction, rule_id: compiled_plan.RuleId, target: []const u8) TransactionError!bool {
        const plan = self.compiledPlan() orelse return false;
        const index: usize = @backingInt(rule_id);
        if (index >= plan.rules.len) return false;
        ...
        if (colon == 0 or colon + 1 == target.len) return false;
        const target_collection = target[0..colon];
        const key = target[colon + 1 ..];
        for (self.regex_target_exclusions.items) |*exclusion| {
            if (!std.ascii.eqlIgnoreCase(exclusion.collection, target_collection)) continue;
            if (!exclusionMatchesRule(plan, rule, exclusion.selector)) continue;
            if (try exclusion.worker.matcher().matches(key)) return true;
        }
        return false;
    }
...
test "runtime rule id and tag exclusions affect only subsequent cursor entries" {
    const input =
        \\SecRule ARGS @rx "id:1,phase:1,ctl:ruleRemoveById=2,ctl:ruleRemoveByTag=skip-me,ctl:ruleRemoveTargetById=4;ARGS:secret,ctl:ruleRemoveTargetByTag=group;REQUEST_HEADERS:authorization,ctl:ruleRemoveTargetById=4;ARGS:/^json\.\d+\.desc$/"
    ...
    try std.testing.expect(try tx.targetExcluded(@fromBackingInt(3), "ARGS:secret"));
    try std.testing.expect(try tx.targetExcluded(@fromBackingInt(3), "request_headers:Authorization"));
    try std.testing.expect(!(try tx.targetExcluded(@fromBackingInt(3), "ARGS:public")));
    try std.testing.expect(try tx.targetExcluded(@fromBackingInt(3), "ARGS:json.12.desc"));
    try std.testing.expect(!(try tx.targetExcluded(@fromBackingInt(3), "ARGS:json.name.desc")));
